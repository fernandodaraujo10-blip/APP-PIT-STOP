...
import { 
  ...
  CheckCircle, Banknote, QrCode, CreditCard, Percent, CheckSquare, Square, ThermometerSun
} from 'lucide-react';
...
export const BookingFlow: React.FC<BookingFlowProps> = ({ onCancel }) => {
  ...
  const handleFinalSubmit = async () => {
    ...
    window.open(`https://wa.me/${SHOP_PHONE}?text=${encodeURIComponent(msg)}`);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-center animate-fade-in">
        <div className="bg-white p-10 rounded-[3rem] shadow-xl w-full max-w-md space-y-6 border border-[#E5E7EB]">
          ...
          <p className="text-slate-text text-sm">Seu horário foi reservado com sucesso. Enviamos os detalhes para a loja.</p>
  ...
  return (
    <div className="h-screen bg-[#F3F4F6] overflow-hidden flex flex-col font-sans">
      {step === 1 && (
        <>
          <Header title="1. Quando deseja vir?" />
          <div className="p-4 space-y-6 animate-fade-in max-w-lg mx-auto flex-1 overflow-y-auto pb-32 scrollbar-hide">
      ...
      {step === 2 && (
        <>
          ...
          <div className="p-4 space-y-4 animate-fade-in max-w-lg mx-auto flex-1 overflow-y-auto pb-32 scrollbar-hide">
            {services.filter(s => s.active).map(s => (
              <div 
                key={s.id} 
                onClick={() => { setSelectedService(s); setStep(3); }} 
                className={`p-5 bg-white rounded-[2rem] border-2 transition-all flex items-center justify-between gap-4 shadow-soft ${
                  selectedService?.id === s.id ? 'border-vivid-blue ring-4 ring-blue-50' : 'border-[#E5E7EB]'
                }`}
              >
              ...
              <div className="space-y-2">
                {DIRT_LEVELS.map(level => (
                  <div 
                    ...
                    className={`p-4 rounded-2xl border-2 flex items-center gap-4 transition-all cursor-pointer ${
                      dirtLevel === level.id ? 'border-vivid-blue bg-blue-50' : 'border-[#E5E7EB] bg-white'
                    }`}
                  >
                ...
                {UPSELL_EXTRAS.map(ex => {
                  ...
                  return (
                    <div 
                      ...
                      className={`p-4 rounded-2xl flex justify-between items-center transition-all cursor-pointer ${
                        isSelected ? 'bg-golden/5' : 'bg-transparent'
                      }`}
                    >
      ...
      {step === 4 && (
        <>
                 ...
                 {[
                   ...
                   <button 
                    ...
                    className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${
                      formData.paymentMethod === m.id ? 'border-vivid-blue bg-blue-50 text-vivid-blue' : 'border-[#E5E7EB] bg-white text-slate-400'
                    }`}
                   >
                     {m.icon}
          ...
          <BottomBar 
            ...
            disabled={!formData.name || !formData.phone || !formData.model || !formData.paymentMethod} 
