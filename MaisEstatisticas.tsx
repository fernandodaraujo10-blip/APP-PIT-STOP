...
export const MaisEstatisticas: React.FC = () => {
  ...
  const { cashback, settings, updateCashback, updateShopSettings, restoreDatabase } = useApp();
  ...
  return (
    <div className="space-y-6 animate-fade-in px-2">
      ...
      <div className="pt-6 border-t pb-10">
        <Button
          fullWidth
          variant="danger"
          onClick={() => { if (window.confirm('Isso irá restaurar todos os serviços padrão. Continuar?')) restoreDatabase(); }}
          className="rounded-[2rem] py-4 text-[11px] font-black uppercase tracking-widest opacity-50 hover:opacity-100 transition-opacity"
        >
          Restaurar Banco de Dados (Fix Bug)
        </Button>
      </div>
    </div>
  );
